namespace Dragonfly.UmbracoSiteTester
{
	...
	public class SiteTesterService
	{
			...
			HttpRenderingService HttpRenderingService)
		{
			...
			var httpService = new HttpRenderingService(dependencies, logger, finalDomain, httpUrl, timeout);
		...
		protected void TestNode(IContent Content)
		{
			if (Content != null && Content.Id > 0)
			{
				...
				else
				{
					...
					{
						_logger.LogError(e, "Error in TestNode for Node #{NodeId}", Content.Id);
					}
					...
					try
					{
						...
						{
							result.ErrorException = renderResult.ErrorException;
						}
					...
					catch (Exception e)
					{
						...
						else
						{
							...
							result.ErrorException = e.InnerException;
						}
					}
					//}
				}
		...
		internal Config GetAppDataConfig()
		{
